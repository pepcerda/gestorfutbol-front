...
const HistoricDirectiva = ({ props }) => {
  ...
  useEffect(() => {
    gestorfutbolService.listHistoricDirectiva().then((data) => {
...
const BaixaDirectivaForm = ({ props }) => {
  ...
  const dataBaixaProps = {
    ...
    classNameError: `${
      isFormFieldInvalid("dataBaixa") ? "formcalendar-invalid" : ""
    }`,
    labelClassName: `${
      isFormFieldInvalid("dataBaixa") ? "form-text-invalid" : ""
    }`,
  };
...
const DirectiuDataForm = ({ props }) => {
  ...
  let nomProps = data.map((directiu, index) => {
    return {
      ...
      className: "directiva-form-inputs",
      onChange: (e) => {
        formikDirectiva.setFieldValue(
          `directiva.directius.${index}.nom`,
          e.target.value
        );
      },
      classNameError: `${
        isFormFieldInvalid(`directiva.directius.${index}.nom`)
          ? "invalid-inputtext"
          : ""
      }`,
      labelClassName: `${
  ...
  let llinatge1Props = data.map((directiu, index) => {
    return {
      ...
      onChange: (e) => {
        formikDirectiva.setFieldValue(
          `directiva.directius.${index}.llinatge1`,
          e.target.value
        );
      },
      classNameError: `${
        isFormFieldInvalid(`directiva.directius.${index}.llinatge1`)
          ? "invalid-inputtext"
          : ""
      }`,
      labelClassName: `${
  ...
  let llinatge2Props = data.map((directiu, index) => {
    return {
      ...
      onChange: (e) => {
        formikDirectiva.setFieldValue(
          `directiva.directius.${index}.llinatge2`,
          e.target.value
        );
      },
    };
  ...
  let nifProps = data.map((directiu, index) => {
    return {
      ...
      onChange: (e) => {
        formikDirectiva.setFieldValue(
          `directiva.directius.${index}.nif`,
          e.target.value
        );
      },
      classNameError: `${
        isFormFieldInvalid(`directiva.directius.${index}.nif`)
          ? "invalid-inputtext"
          : ""
      }`,
      labelClassName: `${
  ...
  const onCellEditComplete = (e) => {
    ...
    formikDirectiva.setFieldValue(
      `directiva.directius.${rowIndex}.${field}`,
      newValue
    );
  };
              ...
              <>
                ...
                <div className="row">
                  <div className="col-12">
                    ...
                    <FormInputText props={rolsProps[idx]}></FormInputText>
                  </div>
                  <div className="col-12">
                    <span className="fw-bold">{t("t.name")}</span>
                    <FormInputText props={nomProps[idx]}></FormInputText>
                    {getFormErrorMessage(`directiva.directius.${idx}.nom`)}
                  </div>
                  <div className="col-12">
                    <span className="fw-bold">{t("t.surname1")}</span>
                    <FormInputText props={llinatge1Props[idx]}></FormInputText>
                    {getFormErrorMessage(
                      `directiva.directius.${idx}.llinatge1`
                    )}
                  </div>
                  <div className="col-12">
                    <span className="fw-bold">{t("t.surname2")}</span>
                    <FormInputText props={llinatge2Props[idx]}></FormInputText>
                  </div>
                  <div className="col-12">
                    <span className="fw-bold">{t("t.nif")}</span>
                    <FormInputText props={nifProps[idx]}></FormInputText>
                    {getFormErrorMessage(`directiva.directius.${idx}.nif`)}
                  </div>
                </div>
              </>
            ) : (
...
const DirectivaPage = ({ props }) => {
  ...
  useEffect(() => {
    gestorfutbolService.checkDirectiva().then((data) => {
      if (data.data) {
        gestorfutbolService.listDirectiva().then((data) => {
  ...
  const saveDirectiva = (data) => {
    ...
    gestorfutbolService.saveDirectiva(data.directiva).then(() => {
      setCaptureDialog(false);
      loadData();
    });
  };
  ...
  const baixaDirectiva = (dataBaixa) => {
    gestorfutbolService.baixaDirectiva(dataBaixa).then(() => {
      createEmptyDirectiva(rolsDirectiu);
    ...
    <div className="container p-2 p-xl-4">
      ...
      >
        <form onSubmit={formikBaixa.handleSubmit}>
          <BaixaContext.Provider value={{ formikBaixa }}>
            <BaixaDirectivaForm />
