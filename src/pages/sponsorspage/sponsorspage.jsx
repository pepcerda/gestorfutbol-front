...
const SponsorDataForm = ({ props }) => {
  ...
  const donacioProps = {
    ...
    classNameError: `${
      isFormFieldInvalid("donacio") ? "invalid-inputnumber" : ""
    }`,
    labelClassName: `${
      isFormFieldInvalid("donacio") ? "form-text-invalid" : ""
    }`,
    disabled: captureDialog.consulta,
  };
      ...
      <div className="row">
        ...
        <div className="row align-items-center align-content-center mt-3 mt-md-0 gap-3">
          ...
          {formikSponsor.values.logo &&
            !esBase64Embebida(formikSponsor.values.logo) && (
          ...
          <div className="col-12 col-md-2 text-center text-md-start form-group mt-3 mt-md-0 ms-3">
            {formikSponsor.values.logoBlob && (
              <>
                ...
                <p>
                  <a href={formikSponsor.values.logoBlob} target="_blank">
                    {t("t.document")}
                  </a>
                </p>
              </>
...
const DuplicaForm = ({ props }) => {
  ...
  const campanyesProps = {
    ...
    optionLabel: "titol",
    optionValue: "id",
    classNameError: `${isFormFieldInvalid("campanya") ? "invalid-select" : ""}`,
    labelClassName: `${
...
const SponsorsPage = ({ props }) => {
  ...
  const donwloadPdfButton = (id) => {
    const props = {
      icon: "pi pi-file",
      onClick: () => {
        gestorfutbolService.getReceipt(id).then((r) => {
          handleDownload(r.data);
        });
      },
      className: "download-button p-0",
    };
    return <BasicButton props={props}></BasicButton>;
  };
  ...
  const accept = () => {
    gestorfutbolService.deleteSponsor(selectedSponsor.id).then(() => {
      setDeleteFlag(true);
    });
  };
  ...
  const onRowEditComplete = (e) => {
    ...
    gestorfutbolService.saveSponsor(newData).then(() => loadLazyData());
  };
  ...
  return (
    <div className="container p-2 p-xl-4">
      ...
      <Dialog
        ...
        onHide={hideDialogDuplica}
        className="campaign-dialog"
      >
        <form onSubmit={formikDuplica.handleSubmit}>
          <DuplicaContext.Provider value={{ formikDuplica, campaigns }}>
            <DuplicaForm />
